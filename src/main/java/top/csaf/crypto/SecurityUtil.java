...
public class SecurityUtil {
  ...
  public static String desEcb(@NonNull final String key, @NonNull final String in, final boolean encrypting, final BlockCipherPadding padding) {
    byte[] keys = ArrayUtil.toBytes(key.toCharArray());
    if (keys.length < 8) {
    ...
    if (encrypting) {
      return Hex.toHexString(desEcb(keys, ArrayUtil.toBytes(in.toCharArray()), encrypting, padding));
    }
    byte[] bytes = desEcb(keys, Hex.decode(in), encrypting, padding);
    return new String(ArrayUtil.toChars(bytes));
  }
  ...
  public static String desCbc(@NonNull final String key, @NonNull final String iv, @NonNull final String in, final boolean encrypting, final BlockCipherPadding padding) {
    ...
    byte[] ivs = ArrayUtil.toBytes(iv.toCharArray());
    if (ivs.length < 8) {
    ...
    if (encrypting) {
      return Hex.toHexString(desCbc(keys, ivs, ArrayUtil.toBytes(in.toCharArray()), encrypting, padding));
    }
